...
// waits for all goroutines to mark finished
func (ctx *ContextStruct) Wait() <-chan int {
	c := make(chan int)
	go func() {
		ctx.wg.Wait()
		close(c)
...
func (ctx *ContextStruct) SetParent(key string, val any) error {
	...
	if ctx.parent_ctx == nil {
		return log_item.NewLogItem("ctx.SetParent").
			Set_level_error().SetMessage("parent context is nil:\nkey: %s\nval: %v", key, val)
	}
	if cptx, ok := ctx.parent_ctx.(Context); ok {
